...
const API_IA = "https://plateforme-biomedicale-production-e0bf.up.railway.app"; 
...
function Plateforme({user,token,organisation,prefInitiales,onLogout}){
  ...
  async function supprimerEquipement(id){if(!window.confirm("Confirmer la suppression ?")) return;await fetch(`${API}/equipements/${id}`,{method:"DELETE",headers:hdrs()});setEquipements(p=>p.filter(e=>e.id!==id));toast("✅ Équipement supprimé.");}
